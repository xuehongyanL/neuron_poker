...
class Player:
    ...
    def initWeight(self):  # 权重的初始化 权重记录的是已经被学习的信息
        self.weight = [[1.0 for _ in range(20)] for stage in range(4)]
    ...
    def initLearner(self):  # 弱学习器的初始化 生成一组参数并用参数来实例化学习器
        ...
        self.learners = [[Learner(params) for params in paramList] for stage in range(4)]
    ...
    def updateWeight(self, newState):  # 更新权重 从[状态]中学习并更新到[权重]
        ...
        if stack > self.state['temp_stack']:
            for i,stage in enumerate(self.state['choice']):
                for choice in stage:
                    for j in range(20):
                        if j in choice:
                            self.weight[i][j] += 0.2
    ...
    def updateState(self, newState):  # 更新状态 每次决策后更新状态
        self.state['choice'][newState['current_stage']].append(newState['newChoices'])
        if newState['fold']:
            self.state['fold'] = 1
    ...
    def action(self, action_space, observation, info):  # pylint: disable=no-self-use
        ...
        actions = [learner.decision(
            equity_alive, action_space).value for learner in self.learners[current_stage]]
        action = Learner.reduce(actions, self.weight[current_stage])
